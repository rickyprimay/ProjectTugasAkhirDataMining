      ...
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('register.post') }}">
          ...
          <div>
            ...
            <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-green-600 focus:border-green-600 block w-full p-2.5" placeholder="ricky" required>
          </div>
          <div>
          ...
          <button type="submit" class="w-full text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Register</button>
          <p class="text-sm font-light text-gray-500">
            Sudah Punya Akun? <a href="{{ route('login') }}" class="font-medium text-green-600 hover:underline">Login</a>
          </p>
        </form>
      </div>
    </div>
